...
def copy_directory(src, dest, retries=3, wait_time=60):
    ...
    logger.error(
        f"Failed to copy directory from {src} to {dest} after {retries} attempts"
    )
    ...
    for task in config["tasks"]:
        ...
        for executed_task in executed_tasks["executed"]:
            if executed_task["task"]["id"] == task["id"]:
                executed_time = datetime.strptime(
                    executed_task["executed_time"], "%Y-%m-%d_%H-%M-%S"
                )
                last_executed_timestamp = executed_time
                logger.debug(
                    f"Found last executed timestamp for task {task['id']}: {last_executed_timestamp}"
                )
        ...
        if last_executed_timestamp:
            ...
            if time_diff_days >= config["missed_task_threshold"]:
                ...
                logger.info(
                    f"Task {task['id']} is missed, has not be executed for {time_diff_days} days, which is greater than the {config['missed_task_threshold']} day threshold."
                )
        else:
            missed_tasks.append(task)
            logger.info(
                f"Task {task['id']} has never been executed, adding to the list."
            )
    return missed_tasks
